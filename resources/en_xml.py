...
class ENXML():
    ...
    def set_activity_type(self, activity_type):
        """ Sets the activity type into the right spot for opportunity or issues"""
        a_list=[]
        for val in self.activity:
            if activity_type.upper() == val[2]:
                a_list.append(val[1])
        if activity_type.upper() == 'O':
            el = self.root.xpath(".//td[contains(@style, 'id: opportunity')]")[0]
        elif activity_type.upper() == 'I':
            el = self.root.xpath(".//td[contains(@style, 'id: issue')]")[0]
        for child in el.getchildren():
            el.remove(child)
        ul = etree.SubElement(el, 'ul')
        for val in a_list:
            etree.SubElement(ul, 'li').text=val
        pass
...
if __name__ == '__main__':
    with open('/Users/Codyc/Desktop/TEST.enex', 'r') as f:
            output = f.read()
    en = ENXML(output)
    en.break_out_tables()    
